...
bool loadKNNDataAndTrainKNN(void) {
    ...
    cv::FileStorage fsClassifications(APP_RUN_PATH + "alpr_config/classifications.xml", cv::FileStorage::READ);        // open the classifications file
    ...
    cv::FileStorage fsTrainingImages(APP_RUN_PATH + "alpr_config/images.xml", cv::FileStorage::READ);              // open the training images file
...
std::vector<PossiblePlate> detectCharsInPlates(std::vector<PossiblePlate> &vectorOfPossiblePlates) {
    ...
    for (auto &possiblePlate : vectorOfPossiblePlates) {            // for each possible plate, this is a big for loop that takes up most of the function
...
#ifdef SHOW_STEPS
        imgContours = cv::Mat(possiblePlate.imgThresh.size(), CV_8UC3, PlateRecognizer::SCALAR_BLACK);
        contours.clear();
        ...
        cv::drawContours(imgContours, contours, -1, PlateRecognizer::SCALAR_WHITE);
...
std::string recognizeCharsInPlate(cv::Mat &imgThresh, std::vector<PossibleChar> &vectorOfMatchingChars) {
    ...
    for (auto &currentChar : vectorOfMatchingChars) {           // for each char in plate
        cv::rectangle(imgThreshColor, currentChar.boundingRect, PlateRecognizer::SCALAR_GREEN, 2);       // draw green box around the char
