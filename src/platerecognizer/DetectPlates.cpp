...
std::vector<PossiblePlate> detectPlatesInScene(cv::Mat &imgOriginalScene) {
    ...
    cv::Mat imgContours(imgOriginalScene.size(), CV_8UC3, PlateRecognizer::SCALAR_BLACK);
    ...
    imgContours = cv::Mat(imgOriginalScene.size(), CV_8UC3, PlateRecognizer::SCALAR_BLACK);
   ...
   // cv::imshow("4a", imgContours);
    ...
    for (unsigned int i = 0; i < vectorOfPossiblePlates.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) {
            cv::line(imgContours, p2fRectPoints[j], p2fRectPoints[(j + 1) % 4], PlateRecognizer::SCALAR_RED, 2);
        }
        //  cv::imshow("4a", imgContours);

        //   std::cout << "possible plate " << i << ", click on any image and press a key to continue . . ." << std::endl;

        //cv::imshow("Plate Image", vectorOfPossiblePlates[i].imgPlate);
        cv::waitKey(0);
    }
...
std::vector<PossibleChar> findPossibleCharsInScene(cv::Mat &imgThresh) {
    ...
    cv::Mat imgContours(imgThresh.size(), CV_8UC3, PlateRecognizer::SCALAR_BLACK);
...
#ifdef SHOW_STEPS
        cv::drawContours(imgContours, contours, i, PlateRecognizer::SCALAR_WHITE);
#endif	// SHOW_STEPS
